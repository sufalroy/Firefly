...
import org.firefly.core.util.Util;
...
public abstract class Camera {
    ...
    protected Camera(Vec3f position, Vec3f forward, Vec3f up){
        ...
        height = BaseContext.getConfig().getWindowHeight();
        ...
        setOriginViewMatrix(new Matrix4f().View(getForward(), getUp()).mul(new Matrix4f().Identity()));
        setXzOriginViewMatrix(new Matrix4f().View(getForward(), getUp()).mul(new Matrix4f().Translation(
    ...
    public void update(){
        ...
        if(input.isMouseButtonHolding(0) || input.isMouseButtonHolding(2))
        {
    ...
    protected  void initfrustumPlanes(){
        //left plane
